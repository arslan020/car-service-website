...
function parseHoursGrid(raw: string): { day: string; hours: string; closed: boolean }[] {
  return raw
    .split("\n")
    .map((l) => l.trim())
    .filter(Boolean)
    .map((line) => {
      ...
      const day = pipe === -1 ? line : line.slice(0, pipe).trim();
      const hours = pipe === -1 ? "" : line.slice(pipe + 1).trim();
...
export function ContactPageClient({ content, editable = false }: { content: ContentMap; editable?: boolean }) {
  ...
  const serviceOptions = useMemo(
    () => fl(content, "contact_service_options", SERVICE_OPTIONS_FALLBACK),
    [content]
  );

  const hoursRows = useMemo(() => parseHoursGrid(f(content, "contact_hours_grid", "")), [content]);

  const defaultClosing = f(content, "status_closing_time", "20:00");
  ...
  function validate() {
    ...
    if (!form.name.trim()) e.name = f(content, "err_name", "Name is required.");
    if (!form.email.trim()) e.email = f(content, "err_email", "Email is required.");
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))
      e.email = f(content, "err_email_fmt", "Enter a valid email address.");
    if (!form.message.trim()) e.message = f(content, "err_message", "Message is required.");
    return e;
  }
                ...
                <div className="flex flex-col gap-1.5 sm:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder={f(
                      content,
                      "ph_message",
                      "Tell us about your vehicle and what you need help with..."
                    )}
                    value={form.message}
            ...
            <div className="flex flex-col gap-5">
              <div className="rounded-2xl border border-[#e8effa] bg-white p-6 shadow-sm">
                ...
                <ul className="mt-5 space-y-4">
                  {hoursRows.map((row) => (
                    <li key={row.day} className="flex items-center justify-between">
                      ...
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-bold ${row.closed ? "bg-red-50 text-red-500" : "bg-emerald-50 text-emerald-700"}`}
                      >
                        {row.hours}
                      </span>
                    </li>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <Link
                    href="/quote"
                    className="flex w-full items-center justify-center rounded-xl bg-[#0F63FF] py-3.5 text-sm font-bold text-white shadow-md transition hover:bg-[#1E6BFF]"
                  >
                    Request a Quote
                  </Link>
                </div>
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="overflow-hidden rounded-2xl border border-[#e8effa] shadow-sm">
            <div className="aspect-video w-full overflow-hidden bg-slate-100">
              <iframe
                title={f(content, "map_iframe_title", "Map")}
