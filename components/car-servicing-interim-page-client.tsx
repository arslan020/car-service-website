...
/** "Title: description" list line → bold title + plain description. */
function itemDisplay(line: string): React.ReactNode {
  const idx = line.indexOf(":");
  if (idx === -1) return line;
  return (
    <>
      <span className="font-semibold text-[#020F3D]">{line.slice(0, idx)}:</span> {line.slice(idx + 1).trim()}
    </>
  );
}
...
export function CarServicingInterimPageClient({ content, editable = false }: { content: ContentMap; editable?: boolean }) {
  ...
  const E = (fieldKey: string, type: "text" | "textarea" = "text") => (
    <EditableText pageKey="car-servicing-interim" fieldKey={fieldKey} value={c[fieldKey] ?? ""} type={type} editable={editable} />
  );
        ...
        <div className="mx-auto max-w-5xl">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              ...
              <p className="mt-3 text-3xl font-extrabold text-[#0F63FF]">{E("price_display")}</p>
              <p className="mt-1 text-sm font-semibold text-[#0F63FF]">{E("price_note")}</p>
              <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-[#e0ebff] bg-white px-4 py-2 shadow-sm">
                ...
                <span className="text-sm text-slate-500">{E("interval_prefix")} <strong className="text-[#020F3D]">{E("interval_time")}</strong> {E("interval_joiner")} <strong className="text-[#020F3D]">{E("interval_miles")}</strong></span>
              </div>
          ...
          <div className="grid gap-10 lg:grid-cols-2">
              ...
              <ul className="mt-6 space-y-4">
                {includes.map((item) => (
                  <li key={item} className="flex items-start gap-3 text-sm text-slate-600">
                    ...
                    <EditableText pageKey="car-servicing-interim" fieldKey="includes_list" value={c.includes_list ?? ""} type="textarea" editable={editable} display={itemDisplay(item)} />
                  </li>
                ))}
              </ul>
            </div>
            <div>
              <div className="rounded-2xl border border-[#e0ebff] bg-[#f4f8ff] p-5">
                <p className="text-xs font-bold uppercase tracking-widest text-[#0F63FF]">{E("upsell_kicker")}</p>
                <p className="mt-2 text-sm text-slate-600 leading-relaxed">
                  {E("upsell_pre")} <strong className="text-[#020F3D]">{E("upsell_bold")}</strong> {E("upsell_post", "textarea")}
                </p>
        ...
        <div className="mx-auto max-w-5xl">
          <div className="overflow-hidden rounded-3xl bg-[#020F3D] px-8 py-10 text-center shadow-xl sm:py-14">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-slate-300">
              {E("bottom_subtitle", "textarea")} {site.addressLines.join(", ")}.
            </p>
