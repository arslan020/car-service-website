...
export function RepairsTyresPageClient({ content, editable = false }: { content: ContentMap; editable?: boolean }) {
  ...
  const E = (fieldKey: string, type: "text" | "textarea" = "text") => (
    <EditableText pageKey="repairs-tyres" fieldKey={fieldKey} value={c[fieldKey] ?? ""} type={type} editable={editable} />
  );
          ...
          <ul className="mt-6 grid gap-3 sm:grid-cols-2">
            {services.map((item) => (
              <li key={item} className="flex items-start gap-3 text-sm text-slate-600">
                ...
                <EditableText pageKey="repairs-tyres" fieldKey="includes_list" value={c.includes_list ?? ""} type="textarea" editable={editable} display={item} />
              </li>
            ))}
          </ul>
        </div>
