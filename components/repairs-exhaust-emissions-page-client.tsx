...
export function RepairsExhaustEmissionsPageClient({ content, editable = false }: { content: ContentMap; editable?: boolean }) {
  ...
  const E = (fieldKey: string, type: "text" | "textarea" = "text") => (
    <EditableText pageKey="repairs-exhaust-emissions" fieldKey={fieldKey} value={c[fieldKey] ?? ""} type={type} editable={editable} />
  );
          ...
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              ...
              <div className="mt-4 inline-flex items-start gap-2 rounded-2xl border border-[#e0ebff] bg-white px-4 py-3 shadow-sm">
                ...
                <span className="text-sm font-semibold text-[#020F3D]">{E("hero_badge")}</span>
              </div>
          ...
          <ul className="mt-6 grid gap-3 sm:grid-cols-2">
            {services.map((item) => (
              <li key={item} className="flex items-start gap-3 text-sm text-slate-600">
                ...
                <EditableText pageKey="repairs-exhaust-emissions" fieldKey="includes_list" value={c.includes_list ?? ""} type="textarea" editable={editable} display={item} />
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="px-4 py-12 sm:py-16">
        <div className="mx-auto max-w-5xl">
          <div className="overflow-hidden rounded-3xl bg-[#020F3D] px-8 py-10 text-center shadow-xl sm:py-14">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-slate-300">{site.addressLines.join(", ")}. {E("bottom_subtitle", "textarea")}</p>
