...
export function BatteryCheckPageClient({ content, editable = false }: { content: ContentMap; editable?: boolean }) {
  ...
  const E = (fieldKey: string, type: "text" | "textarea" = "text") => (
    <EditableText pageKey="battery-check" fieldKey={fieldKey} value={c[fieldKey] ?? ""} type={type} editable={editable} />
  );

  const checks = lines(c.checks_list);
  ...
  return (
    <div className="bg-white">
      ...
      <section className="px-4 py-12 sm:py-16">
        <div className="mx-auto max-w-5xl">
          <p className="text-xs font-bold uppercase tracking-widest text-[#0F63FF]">{E("checks_kicker")}</p>
          <h2 className="mt-1 text-2xl font-extrabold text-[#020F3D] sm:text-3xl">{E("checks_title")}</h2>
          <ul className="mt-6 grid gap-3 sm:grid-cols-2">
            {checks.map((item) => (
              <li key={item} className="flex items-start gap-3 text-sm text-slate-600">
                ...
                <EditableText pageKey="battery-check" fieldKey="checks_list" value={c.checks_list ?? ""} type="textarea" editable={editable} display={item} />
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="px-4 py-12 sm:py-16">
        <div className="mx-auto max-w-5xl">
          <div className="overflow-hidden rounded-3xl bg-[#020F3D] px-8 py-10 text-center shadow-xl sm:py-14">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-slate-300">{E("bottom_subtitle", "textarea")}</p>
