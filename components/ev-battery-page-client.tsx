...
export function EvBatteryPageClient({ content, editable = false }: { content: ContentMap; editable?: boolean }) {
  ...
  const E = (fieldKey: string, type: "text" | "textarea" = "text") => (
    <EditableText pageKey="ev-battery" fieldKey={fieldKey} value={c[fieldKey] ?? ""} type={type} editable={editable} />
  );
          ...
          <ul className="mt-6 grid gap-3 sm:grid-cols-2">
            {checks.map((item) => (
              <li key={item} className="flex items-start gap-3 text-sm text-slate-600">
                ...
                <EditableText pageKey="ev-battery" fieldKey="checks_list" value={c.checks_list ?? ""} type="textarea" editable={editable} display={item} />
              </li>
            ))}
          </ul>
        </div>
