...
export default async function RepairsPage() {
  const c = await getPageContentWithDefaults("repairs");

  const processSteps = [
    { n: 1, title: c.proc_1_title, body: c.proc_1_body },
    { n: 2, title: c.proc_2_title, body: c.proc_2_body },
    { n: 3, title: c.proc_3_title, body: c.proc_3_body },
    ...
    <div className="bg-white">
      <section className="bg-gradient-to-b from-[#eefdff] via-[#f5feff] via-60% to-white px-4 pb-12 pt-16 text-center sm:pt-20">
        <div className="mx-auto max-w-2xl">
          ...
          <div className="mt-7 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            ...
            <a
              href={waUrl("Hi, I need a repair quote please.")}
              target="_blank"
      ...
      <section className="px-4 py-12 sm:py-16">
        <div className="mx-auto max-w-5xl">
          <p className="text-xs font-bold uppercase tracking-widest text-[#3f63ff]">{c.cats_kicker}</p>
          <h2 className="mt-1 text-2xl font-extrabold text-[#101a56] sm:text-3xl">{c.cats_title}</h2>
          <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {REPAIR_CATEGORIES.map((cat) => {
              const title = c[cat.titleKey as keyof typeof c] as string;
              const desc = c[cat.descKey as keyof typeof c] as string;
              const items = fl(c, cat.itemsKey, cat.itemsFb);
              return (
                <Link key={cat.href} href={cat.href} className="group flex flex-col rounded-2xl border border-[#e8effa] bg-white p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:border-[#3f63ff]/30 hover:shadow-[0_6px_24px_rgba(63,99,255,0.1)]">
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="font-bold text-[#101a56] transition-colors group-hover:text-[#3f63ff]">{title}</h3>
                    </div>
                  ...
                  <ul className="mt-4 space-y-1.5 border-t border-[#eef4ff] pt-4">
                    {items.map((item) => (
                      <li key={item} className="flex items-center gap-2 text-xs text-slate-600">
                        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#3f63ff]" />
                        {item}
                      </li>
                    ))}
                  </ul>
      ...
      <section className="bg-[#f4f8ff] px-4 py-10 sm:py-16">
        <div className="mx-auto max-w-5xl">
          <div className="text-center">
            <p className="text-xs font-bold uppercase tracking-widest text-[#3f63ff]">{c.process_kicker}</p>
            <h2 className="mt-1 text-2xl font-extrabold text-[#101a56] sm:text-3xl">{c.process_title}</h2>
          </div>
